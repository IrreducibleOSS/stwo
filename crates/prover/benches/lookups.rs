...
fn bench_gkr_grand_product<B: GkrOps>(c: &mut Criterion, id: &str) {
    let mut group = c.benchmark_group(format!("{id} gkr grand products"));

    let mut rng = SmallRng::seed_from_u64(0);
    for log_size in [16, 20, 24] {
        let layer = Layer::<B>::GrandProduct(gen_random_mle(&mut rng, log_size));
        group.throughput(Throughput::Elements(1 << log_size));
        group.bench_function(format!("2^{log_size}"), |b| {
            b.iter_batched(
                || layer.clone(),
...
fn bench_gkr_logup_multiplicities<B: GkrOps>(c: &mut Criterion, id: &str) {
    let mut group = c.benchmark_group(format!("{id} gkr logup with multiplicities"));

    let mut rng = SmallRng::seed_from_u64(0);
    for log_size in [16, 20, 24] {
        let multiplicities_layer = Layer::<B>::LogUpMultiplicities {
            numerators: gen_random_mle(&mut rng, log_size),
            denominators: gen_random_mle(&mut rng, log_size),
        };
        group.throughput(Throughput::Elements(1 << log_size));
